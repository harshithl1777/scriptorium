...
type CodeTemplatesContextType = {
  isLoading: boolean;
  getAllTemplates: () => Promise<CodeTemplate[]>;
...
const CodeTemplatesContext = createContext<
  CodeTemplatesContextType | undefined
>(undefined);
...
export const TemplatesProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const getAllTemplates = useCallback(async (): Promise<CodeTemplate[]> => {
    setIsLoading(true);
    try {
  ...
  const searchTemplates = async (query: string): Promise<CodeTemplate[]> => {
    setIsLoading(true);
    try {
...
export const useTemplates = (): CodeTemplatesContextType => {
  ...
  if (!context) {
    throw new Error(
      "useCodeTemplates must be used within a CodeTemplatesProvider"
    );
  }
  return context;
};
